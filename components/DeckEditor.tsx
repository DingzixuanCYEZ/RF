...
export const DeckEditor: React.FC<DeckEditorProps> = ({ deck, onUpdateDeck, onBack }) => {
  ...
  const [historyFilter, setHistoryFilter] = useState<'STUDY' | 'EXAM'>('STUDY');
  ...
  const filteredLogs = (deck.sessionHistory || [])
    .filter(log => log.mode === historyFilter)
    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-6 pb-20">
      ...
      {activeTab === 'phrases' ? (
        ...
        <div className="space-y-4">
           {/* Sub-tabs for History Mode */}
           <div className="flex p-1 bg-slate-100 rounded-lg max-w-sm">
             <button
               onClick={() => setHistoryFilter('STUDY')}
               className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-all flex items-center justify-center gap-2 ${historyFilter === 'STUDY' ? 'bg-white shadow text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
             >
               <BookOpen className="w-4 h-4" /> 日常练习
             </button>
             <button
               onClick={() => setHistoryFilter('EXAM')}
               className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-all flex items-center justify-center gap-2 ${historyFilter === 'EXAM' ? 'bg-white shadow text-amber-600' : 'text-slate-500 hover:text-slate-700'}`}
             >
               <GraduationCap className="w-4 h-4" /> 模拟考试
             </button>
           </div>
           ...
           {filteredLogs.length === 0 ? (
             <div className="text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-200 text-slate-400">
               ...
               <p>暂无{historyFilter === 'STUDY' ? '练习' : '考试'}记录</p>
             </div>
           ) : (
             <div className="grid gap-3">
               {filteredLogs.map((log) => {
                  ...
                  return (
                    <div key={log.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between animate-in fade-in slide-in-from-bottom-2">
                       <div className="flex items-center gap-4">
